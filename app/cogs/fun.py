...
class Fun(commands.Cog):
    ...
    @commands.command(
        name='impersonate', aliases=['as'],
        brief="Impersonate another user"
    )
    @commands.guild_only()
    async def impersonate_user(
        ...
        *, message: str
        ...
        delete_webhook = \
            ...
            WHERE channel_id=$1::integer"""
        create_webhook = \
            """INSERT INTO webhooks (channel_id, webhook_url)
        ...
        if sql_webhook is None:
            webhook: discord.Webhook = await ctx.channel.create_webhook(
                name="Sparta impersonate Command", reason="Impersonation Command"
            )
            async with self.bot.db.lock:
                await conn.execute(
